...
class Console:
    ...
    def menu(self):
        ...
        print("16. A lowest cost walk between the given vertices, using a backwards Dijkstra algorithm")
    ...
    def run_menu(self):
        while True:
            ...
            if choice == 0:
                ...
                try:
                    starting = int(input("Choose starting vertex: "))
                    ending = int(input("Choose ending vertex: "))
                    path,cost = self.graph.findLowestCostWalk(starting,ending)
                    # Print the results
                    print(f"The lowest cost walk from {starting} to {ending} is:")
                    print(path)
