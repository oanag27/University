...
// A sample transformer function for transforming values
...
void testReplaceAll()
{
	cout << "Test replace all \n";
	SortedMultiMap sortMap(sortAscendingOrder);
	sortMap.add(1, 10);
	sortMap.add(2, 30);
	sortMap.add(3, 40);
	// Display the initial key-value pairs
	/*
	cout << "Initial pairs:" << endl;
	SMMIterator iterat = sortMap.iterator();
	while (iterat.valid()) {
		cout << "key: " << iterat.getCurrent().first << ",value: " << iterat.getCurrent().second << endl;
		iterat.next();
	}
	*/
	//call the function
	sortMap.replaceAll(transformer);

	// Display the key-value pairs after calling the transformer function
	/*
	cout << "Initial pairs:" << endl;
	SMMIterator iterato = sortMap.iterator();
	while (iterato.valid()) {
		cout << "key: " << iterato.getCurrent().first << ",value: " << iterato.getCurrent().second << endl;
		iterato.next();
	}
	*/
}
