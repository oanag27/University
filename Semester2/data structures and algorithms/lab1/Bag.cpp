...
bool Bag::remove(TElem elem) {
	...
	for (i = new_index_unique; i + 1 < this->size_array_unique; i++)
	{
		this->array_unique[i] = this->array_unique[i + 1];
	}
	this->size_array_unique--;

	for (i = 0; i < this->size_array_positions; i++)
	{
		if (this->array_positions[i] > new_index_unique)
		{
			this->array_positions[i]--;
		}
	}
